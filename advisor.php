		...
		<div class="limiter">
		 <div class="container-advisor">
			 <div class="wrap-advisor">
				 <form action="advisor.php" class='needs-validation form-horizontal  table-striped' id="formsq" role='form' method="post" novalidate>
				 ...
				 <label class='col-form-label label_form_font'  style="text-align: right">สืบค้นโดย</label>
				 		...
				 		<option value="aid">รหัสอาจารย์</option>
				 			<option value="aname">ชื่ออาจารย์</option>
				 ...
				 if(isset($_POST['submit_q']))
				 {
				 	...
				 	{
				 		case 'aid':
				 		$sql = "SELECT AdvisorID AS aid,CONCAT(titlename.TitleName_Th,Fname_Th,' ',Lname_Th) AS aname_th,CONCAT(titlename.TitleName_En,Fname_En,' ',Lname_En) AS aname_en,
				 		faculty.FacultyName_Th AS fname, department.DaptName_Th AS dname
				 		...
				 		WHERE AdvisorID = $keyword
				 		ORDER BY AdvisorID ASC limit {$start} , {$perpage};";

				 		$sql2 = "SELECT AdvisorID AS aid,CONCAT(titlename.TitleName_Th,Fname_Th,' ',Lname_Th) AS aname_th,CONCAT(titlename.TitleName_En,Fname_En,' ',Lname_En) AS aname_en,
				 		faculty.FacultyName_Th AS fname, department.DaptName_Th AS dname
				 		FROM advisor INNER JOIN titlename
				 		ON advisor.TitleName = titlename.TitleNameID LEFT OUTER JOIN faculty
				 		ON advisor.Faculty = faculty.FacultyID LEFT OUTER JOIN department
				 		 ...
				 		 <table class="table table-bordered table-hover" style="width: 100%">
				 			 <thead>
				 				<tr style="text-align: center">
				 					 <th>รหัสอาจารย์</th>
				 					 <th>ชื่ออาจารย์ (ไทย)</th>
				 					 <th>ชื่ออาจารย์ (อังกฤษ)</th>
				 					 <th>คณะ</th>
				 					 <th>สาขาวิชา</th>
				 					 <th>การจัดการ</th>
				 					 </tr>
				 					 ...
				 					 ?>
				 								<tr>
				 								<td style="text-align: center" width="10%"><?php echo $row->aid;?></td>
				 								<td><?php echo $row->aname_th;?></td>
				 								<td><?php echo $row->aname_en;?></td>
				 								<?php if($row->fname=="")
				 									{
				 								...
				 								<img src="public/images/baseline_border_color_black_36dp.png" width="20px" data-toggle="tooltip" data-placement="bottom" title="แก้ไข"  name="edit" value="edit" id="<?php echo $row->aid; ?>" class="edit"/>
				 								 &ensp;
				 									<a  href="del_ad.php?aid=<?=$row->aid;?>" onClick="ts();">
				 									<img src="public/images/baseline_delete_forever_black_36dp.png" width="20px" data-toggle="tooltip" data-placement="bottom" title="ลบ" onClick="return confirm('ต้องการลบข้อมูลอาจารย์ รหัส <?php echo $row->aid;?> ใช่หรือไม่')"/></a>
				 								</td>
				 								</tr>
				 					<?php
				 								 $ct++;
				 ...
				 ?>
				  <nav class="Page navigation example">
				 	<ul class="pagination justify-content-center">
				 		<li class="page-item">
				 			<a class="page-link" href="advisor.php" aria-label="Previous">
				 				...
				 				{
				 				?>
				 					<li class="page-item"><a class="page-link" href="advisor.php?page=<?php echo $i; ?>"><?php echo $i; ?></a></li>
				 				<?php
				 				}
				 				?>
				 				 <li class="page-item">
				 					<a class="page-link" href="advisor.php?page=<?php echo $total_page;?>" aria-label="Next">
				 ...
				 <form class="needs-validation" method="post" action="edit_ad.php" novalidate>
				 <div class="modal-header">
				  <h5 class="modal-title labelH_form_font" id="edit_label">แก้ไขข้อมูลอาจารย์</h5>
				 </div>
				 ...
				 <div class="modal-dialog modal-dialog-centered" role="document">
				 <div class="modal-content">
				 <form class="needs-validation" method="post" action="add_ad.php" novalidate>
				 <div class="modal-header">
				 	<h5 class="modal-title labelH_form_font" id="add_label">เพิ่มข้อมูลอาจารย์</h5>
				 </div>
				 <div class="modal-body">
				 <div class="form-group">
				 	 <label class='col-form-label label_form_font' for="aid" style="text-align: right">รหัสอาจารย์</label>
				 	<input class='form-control ' name="aid"  id="aid"  type='text'  pattern="([0-9]{1,4}$)" required>
