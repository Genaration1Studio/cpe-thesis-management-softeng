  ...
  <div id="main">
		<div class="limiter">
		 <div class="container-fac">
			 <div class="wrap-fac">
				 <form action="faculty.php" class='needs-validation form-horizontal  table-striped' id="formsq" role='form' method="post" novalidate>
				 ...
				 <label class='col-form-label label_form_font'  style="text-align: right">สืบค้นโดย</label>
				 		...
				 		<option value="fid">รหัสคณะ</option>
				 			<option value="fname">ชื่อคณะ</option>
				 ...
				 if(isset($_POST['submit_q']))
				 {
				 	switch($sq)
				 	{
				 		...
				 		$sql = "SELECT faculty.FacultyID AS fid, faculty.FacultyName_Th AS fname_th, faculty.FacultyName_En AS fname_en
				 				FROM faculty WHERE faculty.FacultyID = $keyword
				 				ORDER BY FacultyID ASC limit {$start} , {$perpage};";

				 		$sql2 = "SELECT faculty.FacultyID AS fid, faculty.FacultyName_Th AS fname_th, faculty.FacultyName_En AS fname_en
				 				FROM faculty WHERE faculty.FacultyID = $keyword
				 				ORDER BY FacultyID ASC;";
				 		break;
				 		case 'fname':
				 		$sql = "SELECT faculty.FacultyID AS fid, faculty.FacultyName_Th AS fname_th, faculty.FacultyName_En AS fname_en
				 				FROM faculty WHERE CONCAT(FacultyName_Th,' ',FacultyName_En) LIKE '%$keyword%'
				 				ORDER BY FacultyID ASC limit {$start} , {$perpage};";
				 					 ...
				 					 ?>
				 							 ...
				 							 <?php if($row->fname_th=="")
				 								...
				 								<?php
				 									...
				 									{
				 								 ?>		<td ><?php echo $row->fname_en;?></td>
				 								 <?php
				 									}
				 								 ?>
				 								<td style="text-align: center">
				 								<img src="public/images/baseline_border_color_black_36dp.png" width="20px" data-toggle="tooltip" data-placement="bottom" title="แก้ไข"  name="edit" value="edit" id="<?php echo $row->fid; ?>" class="edit"/>
				 								 &ensp;
				 									<a  href="del_fac.php?fid=<?=$row->fid;?>" onClick="ts();">
				 									<img src="public/images/baseline_delete_forever_black_36dp.png" width="20px" data-toggle="tooltip" data-placement="bottom" title="ลบ" onClick="return confirm('ต้องการลบข้อมูลคณะ รหัส <?php echo $row->fid;?> ใช่หรือไม่')"/></a>
				 								</td>
				 								</tr>
				 					<?php
				 								 $ct++;
				 ...
				 ?>
				  <nav class="Page navigation example">
				 	<ul class="pagination justify-content-center">
				 		<li class="page-item">
				 			<a class="page-link" href="faculty.php" aria-label="Previous">
				 				...
				 				{
				 				?>
				 					<li class="page-item"><a class="page-link" href="faculty.php?page=<?php echo $i; ?>"><?php echo $i; ?></a></li>
				 				<?php
				 				}
				 				?>
				 				 <li class="page-item">
				 					<a class="page-link" href="faculty.php?page=<?php echo $total_page;?>" aria-label="Next">
				 ...
				 <form class="needs-validation" method="post" action="edit_fac.php" novalidate>
				 <div class="modal-header">
				  <h5 class="modal-title labelH_form_font" id="edit_label">แก้ไขข้อมูลคณะ</h5>
				 </div>
				 ...
				 <div class="modal-dialog modal-dialog-centered" role="document">
				 <div class="modal-content">
				 <form class="needs-validation" method="post" action="add_fac.php" novalidate>
				 <div class="modal-header">
				 	<h5 class="modal-title labelH_form_font" id="add_label">เพิ่มข้อมูลคณะ</h5>
				 </div>
				 <div class="modal-body">
				 	<div class="form-group">
				 			<label for="fid" class="col-form-label label_form_font">รหัสคณะ</label>
				 			<input type="text" class="form-control" id="fid" name="fid" pattern="([0-9]{1,4}$)"  required>
				 <div class="invalid-feedback">
				 				 กรุณากรอกรหัสคณะ เป็นตัวเลข 0-9
				 		</div>
				 </div>
				 	<div class="form-group">
				 			<label for="fname" class="col-form-label label_form_font">ชื่อคณะ(ภาษาไทย)</label>
				 			<input type="text" class="form-control" id="fname_th" name="fname_th" pattern="^[ก-๙0-9-_\.]{1,255}$" required>
				 	<div class="invalid-feedback">
				 				 กรุณากรอกชื่อคณะ(ภาษาไทย)
				 		</div>
				 </div>
				 	<div class="form-group">
				 			<label for="fname" class="col-form-label label_form_font">ชื่อคณะ(ภาษาอังกฤษ)</label>
				 			<input type="text" class="form-control" id="fname_en" name="fname_en" pattern="^[a-zA-Z0-9-_\. ]{1,255}$" required>
				 	<div class="invalid-feedback">
				 				 กรุณากรอกชื่อคณะ(ภาษาอังกฤษ)
				 		</div>
				 </div>
				 </div>
				 <div class="modal-footer">
