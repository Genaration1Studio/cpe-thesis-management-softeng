		...
		<div class="limiter">
		 <div class="container-course ">
			 <div class="wrap-course">
				 <form action="course.php" class='needs-validation form-horizontal  table-striped' id="formsq" role='form' method="post" novalidate>
			 	...
			 	<label class='col-form-label label_form_font'  style="text-align: right">สืบค้นโดย</label>
			 			...
			 			<option value="cosid">รหัสหลักสูตร</option>
			 				<option value="cosname">ชื่อหลักสูตร</option>
			 	...
			 	if(isset($_POST['submit_q']))
			 	{
			 		switch($sq)
			 		{
			 			...
			 			$sql = "SELECT course.CourseID AS cosid,course.Course AS cosname
			 					FROM `course` WHERE course.CourseID = $keyword
			 					ORDER BY course.CourseID ASC limit {$start} , {$perpage};";

			 			$sql2 = "SELECT course.CourseID AS cosid,course.Course AS cosname
			 					 FROM `course` WHERE course.CourseID = $keyword
			 					 ORDER BY course.CourseID ASC;";
			 			break;
			 			case 'cosname':
			 			$sql = "SELECT course.CourseID AS cosid,course.Course AS cosname
			 					FROM `course` WHERE course.Course LIKE '%$keyword%'
			 					ORDER BY course.CourseID ASC limit {$start} , {$perpage};";
			 			 ...
			 			 <table class="table table-bordered table-hover" style="width: 100%">
			 				 <thead>
			 					<tr style="text-align: center">
			 						 ...
			 						 ?>
			 									<tr>
			 									<td style="text-align: center"><?php echo $row->cosid;?></td>
			 									<td><?php echo $row->cosname;?></td>
			 									<td style="text-align: center">
			 									<img src="public/images/baseline_border_color_black_36dp.png" width="20px" data-toggle="tooltip" data-placement="bottom" title="แก้ไข"  name="edit" value="edit" id="<?php echo $row->cosid; ?>" class="edit"/>
			 									 &ensp;
			 										<a  href="del_cos.php?cosid=<?=$row->cosid;?>" onClick="ts();">
			 										<img src="public/images/baseline_delete_forever_black_36dp.png" width="20px" data-toggle="tooltip" data-placement="bottom" title="ลบ" onClick="return confirm('ต้องการลบข้อมูลหลักสูตร รหัส <?php echo $row->cosid;?> ใช่หรือไม่')"/></a>
			 									</td>
			 									</tr>
			 						<?php
			 									 $ct++;
			 ...
			 ?>
			 	 <nav class="Page navigation example">
			 		<ul class="pagination justify-content-center">
			 			<li class="page-item">
			 				<a class="page-link" href="course.php" aria-label="Previous">
			 					...
			 					{
			 					?>
			 						<li class="page-item"><a class="page-link" href="course.php?page=<?php echo $i; ?>"><?php echo $i; ?></a></li>
			 					<?php
			 					}
			 					?>
			 					 <li class="page-item">
			 						<a class="page-link" href="course.php?page=<?php echo $total_page;?>" aria-label="Next">
			 ...
			 <form class="needs-validation" method="post" action="edit_cos.php" novalidate>
			 	<div class="modal-header">
			 	 <h5 class="modal-title labelH_form_font" id="edit_label">แก้ไขข้อมูลหลักสูตร</h5>
			 	</div>
			 ...
			 <div class="modal-dialog modal-dialog-centered" role="document">
			 <div class="modal-content">
			 <form class="needs-validation" method="post" action="add_cos.php" novalidate>
			 	<div class="modal-header">
			 		<h5 class="modal-title labelH_form_font" id="add_label">เพิ่มข้อมูลหลักสูตร</h5>
			 	</div>
			 	<div class="modal-body">

			 		<div class="form-group">
			 				<label for="cosid" class="col-form-label label_form_font">รหัสหลักสูตร</label>
			 				<input type="text" class="form-control" id="cosid" name="cosid" pattern="([0-9]{1,2}$)"  required>
			 	<div class="invalid-feedback">
			 		...
			 		<div class="form-group">
			 				...
			 				<input type="text" class="form-control" id="cosname" name="cosname" required>
			 		<div class="invalid-feedback">
